    ...
    <div className="container">
      <div className="header">
        ...
        <div className="top-buttons">
          <Link to="/admin/events/create" className="btn primary">+ Thêm sự kiện</Link>
        </div>
      </div>
            ...
            {loading ? (
              ...
              events.map((event) => (
                <tr key={event.id}>
                  ...
                  <td>{new Date(event.startDatetime).toLocaleString('vi-VN')}</td>
                  <td>{new Date(event.endDatetime).toLocaleString('vi-VN')}</td>
                  <td>{getStatusBadge(event.status)}</td>
                  <td>
                    <Link className="btn" to={`/admin/events/${event.id}`}>Chi tiết</Link>
                    <Link className="btn" to={`/admin/events/edit/${event.id}`}>Sửa</Link>
